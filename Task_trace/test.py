...
def trace_ray(start_point,start_dir,surfaces,n_func):
    ...
    for i, surf in enumerate(surfaces):
        if surf.type == '像面':
            if abs(D[2]) < 1e-12:
                print("The light get wrong direction in the image plane")
                continue
        ...
        if surf.type == '球面':
            ...
            if abs(D[2]) < 1e-12:
                print("The light get wrong direction in the Stop")
                continue
            t = (surf.z_vertex - P[2]) / D[2]
        ...
        # 检查 t 的有效性
        if t is None or t <= 0:
            print(f"表面 {i} ({surf.type}) 无有效交点，追迹终止")
            break
...
if __name__ == "__main__":
    ...
    points = result["the point of each surface"]
    dirs = result["the direction of each surface"]
